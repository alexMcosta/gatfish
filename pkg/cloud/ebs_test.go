...
type MockBlankEC2API struct {
	ec2iface.EC2API       // embedding of the interface is needed to skip implementation of all methods
	DescribeVolumesMethod func(*ec2.DescribeVolumesInput) (*ec2.DescribeVolumesOutput, error)
}

func (m *MockBlankEC2API) DescribeVolumes(in *ec2.DescribeVolumesInput) (*ec2.DescribeVolumesOutput, error) {
	if m.DescribeVolumesMethod != nil {
		return m.DescribeVolumesMethod(in)
	}
	volumeID := "vol-1234567"

	return &ec2.DescribeVolumesOutput{Volumes: []*ec2.Volume{{VolumeId: &volumeID}}}, nil
}

func TestEBSCompliance(t *testing.T) {

	t.Run("test generic tags", func(t *testing.T) {
		//Mock the service
		mockSvc := &MockEC2API{}
	...
	t.Run("Test if Volume has no tags", func(t *testing.T) {
		mockSvc := &MockBlankEC2API{}
		want := make(map[string][]string)
		//Mock the output of resources resources/tags.go
		tags := make(map[string]string)
		tags["Notname"] = "DIRE"
